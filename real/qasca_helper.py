...
def worker_submit_answers(assignment, tasks, quality, assign_scheme_tbw, resList, ans): # update prococessing
    if len(assignment) != 0:
        # print assignment
        for index in range(len(assignment)):
            for key in assignment[index]:
                if len(assignment[index][key]) != 0: # check
                    ...
                    qua = quality[key]
                    answer = find_answer(key, task, ans)
...
def obtain_available_workers(batch_workers, assignment):
    ava_workers = batch_workers
    for ass_index in range(len(assignment)):
        for ass_key in assignment[ass_index]:
            if len(assignment[ass_index][ass_key]) != 0 and ass_key in ava_workers:
                ava_workers.remove(ass_key)
    return ava_workers
